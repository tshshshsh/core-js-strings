...
function removeFirstOccurrences(str, value) {
  const replacementIndex = str.indexOf(value);
  if (replacementIndex === -1) return str;
  return (
    str.slice(0, replacementIndex) + str.slice(replacementIndex + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const replacementIndex = str.lastIndexOf(value);
  if (replacementIndex === -1) return str;
  return (
    str.slice(0, replacementIndex) + str.slice(replacementIndex + value.length)
  );
...
function isPalindrome(str) {
  const cleanedStr = str.replaceAll(/\W/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  return sentence
    ...
    .reduce(
      (maxLength, curWord) =>
        curWord.length > maxLength.length ? curWord : maxLength,
      ''
    );
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((char) => {
      ...
      const newChar = alphabet[(index + 13) % 26];
